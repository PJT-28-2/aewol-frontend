...
const handleVerifyCode = async () => {
  ...
  try {
    ...
    codeMessage.value = {
      type: 'error',
      text: message === OTP_UNAVAILABLE_MESSAGE
        ? `${message} 새 인증번호를 다시 요청해주세요.`
...
const handleResetPassword = async () => {
  ...
  if (!isValidPassword(newPassword.value)) {
    submitError.value = '영문, 숫자, 특수문자만 사용해 2가지 조합은 10~20자, 3가지 조합은 8~20자로 입력해주세요'
    return
  }
  ...
  try {
    ...
    const message = error.response?.data?.message
    if (message === RESET_TOKEN_INVALID_MESSAGE) {
      resetToken.value = ''
      verificationCode.value = ''
      newPassword.value = ''
      newPasswordConfirm.value = ''
      ...
      codeMessage.value = {
        type: 'error',
        text: '인증 시간이 만료되었습니다. 새 인증번호를 다시 요청해주세요.',
      }
    } else {
      submitError.value = message ?? '비밀번호 변경에 실패했습니다'
    }
  } finally {
    isLoading.value = false
  }
